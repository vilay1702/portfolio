...
import { PoRData } from "../../shared/types";
  ...
  return (
    <div
      ...
      className={`group relative overflow-hidden rounded-lg p-4 sm:p-6 lg:p-8 transition-all duration-300 hover:scale-[1.02] ${
        theme === "light"
          ? "bg-gradient-to-br from-white to-purple-50 shadow-xl hover:shadow-2xl border border-gray-200"
          : "bg-gradient-to-br from-gray-800 to-purple-900/20 shadow-xl hover:shadow-2xl border border-gray-700"
      }`}
    >
      {/* Gradient overlay on hover */}
      <div
        className={`absolute inset-0 bg-gradient-to-r from-transparent to-transparent group-hover:from-purple-500/5 group-hover:to-pink-500/5 transition-all duration-300 rounded-lg`}
      ></div>
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-start justify-between mb-3 sm:mb-4 lg:mb-6">
          <div className="flex-1">
            ...
            <div
              className={`text-xs sm:text-sm font-medium px-2 sm:px-3 py-1 sm:py-2 rounded inline-block bg-gradient-to-r from-purple-500 to-pink-500 text-white shadow-md`}
            >
              {duration}
            </div>
          </div>
...
const PoR = () => {
  ...
  return (
    <>
      <section id="por" className="font-mono relative">
        {/* Background decoration */}
        <div className="absolute inset-0 -z-10">
          <div
            className={`absolute bottom-1/3 right-10 w-28 h-28 rounded-full opacity-5 ${
              theme === "light" ? "bg-purple-500" : "bg-purple-400"
            } blur-3xl`}
          ></div>
        </div>
        ...
        <section>
          ...
          <div className="px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 lg:gap-8 max-w-6xl mx-auto">
              {POSITION_OF_RESPONSIBILITY.map((data, index) => {
